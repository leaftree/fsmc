 ...
 * return value
 */
static int append_section_to_config(fsmc_config_t *conf, conf_section_t *section)
{
 ...
 * @file: 配置文件名称，带路径
 *
 * 如果@file为空，测尝试打开默认配置/etc/fsmc.ini
 *
 * return value:
 *  NULL: 打开文件失败
 *  !(NULL): @file的文件指针
 ...
 */
static int parase_conf_file(fsmc_config_t *conf)
{
	int retval = 0;
	int has_section = 0;
	char line[512] = "";
...
int load_config(char *file, fsmc_config_t *conf)
{
	...
	if(parase_conf_file(conf) != RETURN_SUCCESS)
		return(RETURN_FAILURE);
